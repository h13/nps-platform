...
> **目的**: Qualtrics 代替として、NPS 計測に特化した軽量プラットフォームを自作する。
...
3. 回答データを D1 に蓄積し、日次で BigQuery に Sync（BigQuery 側の設計は外部チーム担当）
...
- リアルタイム分析（日次バッチで十分）
- BigQuery テーブル設計・Looker Studio ダッシュボード構築（外部チーム担当）
...
| 日次バッチ | Workers Cron Trigger | D1 → BigQuery Sync / Spreadsheet → D1 Sync |
...
[Cloudflare Workers]  ─── 単一プロジェクト、単一ドメインで全て完結
  ...
  └─ [Cron: 毎日 AM 3:00 JST] D1 → BigQuery 日次 Sync
  │
  └─→ [D1]
...
│   │   └── bigquery-sync.ts      # Cron: D1 → BigQuery 日次 Sync
│   ├── middleware/
...
crons = [
  ...
  "0 18 * * *"     # 毎日 18:00 UTC (AM 3:00 JST): D1 → BigQuery Sync
]
...
import { handleBigQuerySync } from './services/bigquery-sync';
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    switch (event.cron) {
      case '0 * * * *':
        ...
        break;
      case '0 18 * * *':
        // 毎日 18:00 UTC: D1 → BigQuery Sync + 失敗メールリトライ
        ctx.waitUntil(handleBigQuerySync(env));
        break;
    }
  },
};
...
## Cron Trigger: 日次 BigQuery Sync

### src/services/bigquery-sync.ts

**処理フロー**:
```
1. D1 から synced_at IS NULL の nps_responses を SELECT（最大 10,000 件）
2. NDJSON 形式に変換
3. Google Service Account JWT 認証で access_token 取得
4. BigQuery Load API でアップロード
5. 成功 → 対象レコードの synced_at を更新
6. 失敗メールリトライ: status = 'failed' AND send_attempts < 3 のレコードを再送信
7. エラー → Slack 通知
```

### 外部チームへの引き継ぎ: NDJSON スキーマ
...
```json
{
  ...
  "nps_score": 8,
  "segment": "passive",
  "answers": "{\"q_nps\":8,\"q_satisfaction\":4,\"q_category\":[\"product\",\"support\"],\"q_reason\":\"サポートが丁寧でした\"}",
  "page_url": null,
  "scroll_percent": null,
  "dwell_seconds": null,
  "stage": "Closed Won",
...
**BigQuery チームへの補足**:
- `answers` は JSON 文字列。BigQuery では `JSON_EXTRACT` / `JSON_EXTRACT_ARRAY` で個別設問を抽出
- 設問の id（`q_nps`, `q_category` 等）は Spreadsheet で管理されており変更の可能性がある
- `nps_score` と `segment` は集計用に独立カラムとして提供
- 設問の追加・削除があっても NDJSON スキーマ自体は変わらない（answers の中身が変わるだけ）
...
| BQ_SERVICE_ACCOUNT_JSON | BigQuery 認証用サービスアカウント JSON |
| SLACK_WEBHOOK_URL | エラー通知用 Slack Webhook URL |

**補足**: `GOOGLE_SERVICE_ACCOUNT_JSON` と `BQ_SERVICE_ACCOUNT_JSON` は同一のサービスアカウントでもよい。その場合は IAM で Sheets API readonly + BigQuery dataEditor のロールを両方付与する。
...
1. `src/services/bigquery-sync.ts` 実装
2. Slack エラー通知
3. Cron Trigger 動作確認
4. 外部チームに NDJSON スキーマ共有

### Phase 7: SF Flow 連携 + 本番
