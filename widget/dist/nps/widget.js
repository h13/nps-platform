...
(() => {
  function y(e, s) {
    ...
    function d() {
      if (o) return;
      (e.operator === 'OR' ? t || r : t && r) && ((o = !0), l(), s());
    }
    function a() {
      ((window.scrollY + window.innerHeight) / document.documentElement.scrollHeight) * 100 >=
        e.scrollPercent && ((t = !0), d());
    }
    let i = setInterval(() => {
      (Date.now() - n) / 1e3 >= e.dwellSeconds && ((r = !0), d());
    }, 1e3);
    window.addEventListener('scroll', a, { passive: !0 });
    function l() {
      (window.removeEventListener('scroll', a), clearInterval(i));
    }
    return l;
  }
  var L = `
...
`;
  function C(e, s) {
    ...
    ((o.textContent = L),
      r.appendChild(o),
      (t.style.cssText = 'position:fixed;bottom:0;right:0;z-index:2147483647;'));
    let n = e.widget_primary_color || '#2563EB',
      d = e.widget_bg_color || '#FFFFFF',
      a = e.widget_text_color || '#1F2937',
      i = document.createElement('div');
    ((i.className = 'overlay'),
      i.style.setProperty('--primary', n),
      i.style.setProperty('--bg', d),
      i.style.setProperty('--text', a));
    let l = {},
      c = document.createElement('div');
    c.className = 'header';
    let w = document.createElement('h2');
    w.textContent = e.survey_title || '';
    let x = document.createElement('button');
    ((x.className = 'close-btn'),
      (x.textContent = '\xD7'),
      x.addEventListener('click', () => s.onClose()),
      c.appendChild(w),
      c.appendChild(x),
      i.appendChild(c));
    let f = document.createElement('div');
    f.className = 'body';
    let v = document.createElement('form');
    v.addEventListener('submit', (m) => {
      m.preventDefault();
      let p = !0;
      for (let u of e.questions) {
        let g = r.getElementById('err-' + u.id);
        if ((g && g.classList.remove('visible'), u.required)) {
          let h = l[u.id];
          (h == null || h === '' || (Array.isArray(h) && h.length === 0)) &&
            (g &&
              ((g.textContent = '\u3053\u306E\u9805\u76EE\u306F\u5FC5\u9808\u3067\u3059'),
      ...
      p &&
        ...
        s
          ...
          .then(() => {
            ...
            ((u.className = 'thanks'), (u.innerHTML = '<div class="check">\u2713</div>'));
            let g = document.createElement('p');
            ((g.textContent = e.thanks_message || ''),
              u.appendChild(g),
              i.appendChild(u),
              setTimeout(() => {
                ((i.style.animation = 'fadeOut 0.3s ease forwards'),
                  setTimeout(() => s.onClose(), 300));
              }, 2e3));
          })
          .catch(() => {
            ((b.disabled = !1), (b.textContent = '\u9001\u4FE1\u3059\u308B'));
          }));
    });
    for (let m of e.questions) {
      let p = document.createElement('div');
      p.className = 'question';
      let u = document.createElement('div');
      if (((u.className = 'q-text'), (u.textContent = m.text), m.required)) {
        let h = document.createElement('span');
        ((h.className = 'required-mark'), (h.textContent = '*'), u.appendChild(h));
      }
      switch ((p.appendChild(u), m.type)) {
        case 'nps_score':
          p.appendChild(N(m, l));
          break;
        case 'rating':
          p.appendChild(H(m, l, n));
          break;
        case 'free_text':
          p.appendChild(P(m, l));
          break;
      ...
      ((g.className = 'error-msg'), (g.id = 'err-' + m.id), p.appendChild(g), v.appendChild(p));
    }
  ...
  function T(e) {
    return e <= 6 ? 'nps-low' : e <= 8 ? 'nps-mid' : 'nps-high';
  }
  function N(e, s) {
    let t = document.createElement('div'),
    ...
    for (let o = 0; o <= 10; o++) {
      ...
      ((n.type = 'button'),
        (n.className = `score-btn ${T(o)}`),
        (n.textContent = String(o)),
        n.addEventListener('click', () => {
          (r.querySelectorAll('.score-btn').forEach((d) => d.classList.remove('selected')),
            n.classList.add('selected'),
            (s[e.id] = o));
        }),
        r.appendChild(n));
    }
    return (t.appendChild(r), E(t, e.min_label, e.max_label), t);
  }
  function H(e, s, t) {
    ...
    for (let a = n; a <= d; a++) {
      ...
      ((i.type = 'button'),
        (i.className = 'score-btn rating-btn'),
        (i.textContent = String(a)),
        i.addEventListener('click', () => {
          (o.querySelectorAll('.score-btn').forEach((l) => l.classList.remove('selected')),
            i.classList.add('selected'),
            (s[e.id] = a));
        }),
        o.appendChild(i));
    }
    return (r.appendChild(o), E(r, e.min_label, e.max_label), r);
  }
  function P(e, s) {
    let t = document.createElement('textarea');
    return (
      e.placeholder && (t.placeholder = e.placeholder),
      e.max_length && (t.maxLength = e.max_length),
  ...
  function R(e, s) {
    ...
    ((r.value = ''),
      (r.textContent = '\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044'),
      t.appendChild(r));
    for (let o of e.options ?? []) {
      let n = document.createElement('option');
      ((n.value = o.value), (n.textContent = o.label), t.appendChild(n));
    }
    return (
      t.addEventListener('change', () => {
        s[e.id] = t.value || void 0;
      }),
      t
    );
  }
    ...
    for (let r of e.options ?? []) {
      ...
      ((n.type = 'checkbox'),
        ...
        n.addEventListener('change', () => {
          let a = t.querySelectorAll('input:checked'),
            i = [];
          (a.forEach((l) => i.push(l.value)), (s[e.id] = i.length > 0 ? i : void 0));
        }));
      let d = document.createElement('span');
      ((d.textContent = r.label), o.appendChild(n), o.appendChild(d), t.appendChild(o));
    }
    return t;
  }
  function z(e, s) {
  ...
  function E(e, s, t) {
    ...
    ((n.textContent = t || ''), r.appendChild(o), r.appendChild(n), e.appendChild(r));
  }
  var k = 'nps_responded_at',
    _ = 'nps_show_count';
  function D(e) {
    try {
  ...
  (function () {
    let s = window.NpsWidget;
    if (!s) return;
    let { endpoint: t, triggers: r, display: o } = s,
      n = I(k);
    if (n) {
      let a = Date.now() - parseInt(n, 10),
        i = (o.cooldownDays || 90) * 24 * 60 * 60 * 1e3;
      if (a < i) return;
    }
    let d = D(_);
    d >= (o.maxShowCount || 3) ||
      fetch(`${t}/nps/config`)
        .then((a) => {
          if (!a.ok) throw new Error(`Config fetch failed: ${a.status}`);
          return a.json();
        })
        .then((a) => {
          let i = Date.now(),
          ...
          function x() {
            c ||
              ...
              (c = C(a, {
                onSubmit: async (f) => {
                  let v = Math.round((Date.now() - i) / 1e3),
                    b = await fetch(`${t}/nps/response`, {
                      method: 'POST',
                      ...
                      body: JSON.stringify({
                        ...
                        scroll_percent: Math.round(l),
                        dwell_seconds: v,
                        user_agent: navigator.userAgent,
                      }),
                    });
                  if (!b.ok) throw new Error(`Response submit failed: ${b.status}`);
                  S(k, String(Date.now()));
                },
                onClose: () => {
                  c && (c.remove(), (c = null));
          ...
          w = y(r, () => {
            let f = o.delay || 0;
            f > 0 ? setTimeout(x, f) : x();
          });
        })
        .catch((a) => {
          console.error('[NPS Widget]', a);
        });
  })();
})();
